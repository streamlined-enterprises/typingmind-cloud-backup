...
if (window.typingMindCloudSync) {
  ...
  class Logger {
    ...
    loadEruda() {
      ...
      // SECURITY FIX (Medium): SRI hash pins the exact Eruda build.
      script.integrity = "sha384-w/A/l37lHs1WXFyUDoQGwc84lCo9p/UHsFj0TCILFJGchIMGYg7vd/5i/BpjObz1";
  ...
  class CryptoService {
    ...
    async deriveKey(password) {
      // SECURITY FIX (High): replaced single SHA-256 with PBKDF2 (100 000 iterations, SHA-256).
      // A random 16-byte salt is generated once per session and stored in this.pbkdf2Salt so
      // that encrypt + decrypt within the same session share the same derived key, while a
      // cold attacker cannot pre-compute a rainbow table without the salt.
      // The derived key is still cached in-memory so we only pay the 100k-iteration cost once.
      const now = Date.now();
  ...
  class S3Service extends IStorageProvider {
    ...
    async loadSDK() {
      ...
      return new Promise((resolve, reject) => {
        ...
        // SECURITY FIX (Medium): pin SRI hash so a compromised CDN cannot inject
        // malicious code.  Recompute if you upgrade the SDK version.
        script.integrity = "sha384-bsVTXMksIT73n577G/meHKgRRSzJu77AotDGo6x838B7oRUm9U+NvG/o2FLodTic";
        script.crossOrigin = "anonymous";
        script.onload = () => {
          this.sdkLoaded = true;
          resolve();
      ...
      return retryAsync(
        async () => {
          try {
            ...
            // SECURITY FIX (Low): metadata.json is now encrypted like all other
            // items, so key names / timestamps are not exposed to bucket readers.
            const body = isMetadata
              ? await this.crypto.encrypt(data, "metadata")
...
async download(key, isMetadata = false) {
  return retryAsync(
    async () => {
      ...
      if (isMetadata) {
        // SECURITY FIX (Low): metadata was previously stored as plaintext JSON;
        // it is now AES-GCM encrypted.  We attempt decryption first and fall
        // back to plain-JSON parsing for legacy (pre-patch) data migration.
        let jsonString;
        try {
          jsonString = await this.crypto.decrypt(bodyBytes, "metadata");
        } catch (_) {
          // Legacy plaintext fallback for first-run migration.
  ...
  class GoogleDriveService extends IStorageProvider {
    ...
    // SECURITY FIX (Medium): _loadScript now accepts an optional SRI integrity
    // hash.  When provided the browser will refuse to execute the script if it
    // does not match, protecting against CDN compromise.
    async _loadScript(id, src, integrity = null) {
    ...
    async upload(key, data, isMetadata = false, itemKey = null) {
      return this._operationWithRetry(async () => {
        ...
        // SECURITY FIX (Low): metadata.json encrypted at rest on Google Drive.
        const body = isMetadata
          ? await this.crypto.encrypt(data, "metadata")
          : key.startsWith("attachments/")
            ? await this.crypto.encryptBytes(data) 
    ...
    async download(key, isMetadata = false) {
      return this._operationWithRetry(async () => {
        ...
        // SECURITY FIX (Low): metadata is now encrypted; decrypt and parse.
        // Falls back to plain JSON parsing for legacy migration.
  ...
  class CloudSyncApp {
    ...
    async setupSyncButtonObserver() {
      ...
      return new Promise((resolve) => {
        ...
        // UI-RELIABILITY FIX: observe <html> (documentElement) instead of just
        // <body> so we catch mutations that happen before <body> is fully
        // populated, e.g. during React's initial server-rendered HTML swap.
        ...
        // UI-RELIABILITY FIX: extended from 10 s → 30 s to accommodate slow
        // machines and cold React hydration cycles.
        setTimeout(() => {
          observer.disconnect();
          resolve();
        }, 30000);
...
async loadTombstoneList(modal) {
  ...
  // UI-RELIABILITY FIX: TypingMind is a React SPA. The sidebar nav buttons are
  // rendered asynchronously after React hydration, which can finish *after* this
  // script executes.  Guard with DOMContentLoaded + an extended observer window.
  function startApp() {
    app.initialize();

    // Persistent re-insert guard: TypingMind sometimes unmounts/remounts the
    // sidebar on navigation (e.g. opening a new chat).  Poll every 3 s to make
    // sure the Sync button stays present even after React re-renders the nav.
    setInterval(() => {
      if (!document.querySelector('[data-element-id="workspace-tab-cloudsync"]')) {
  ...
  if (document.readyState === "loading") {
    ...
    // Document already parsed — still defer one tick so React can flush.
